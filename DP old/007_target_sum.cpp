...
// Count the number of ways to assign signs to elements of array to make given sum of array
// 2 partitions: -ve sign, +ve sign
...
int TargetSumOptimized(vector<int> arr, int n, int diff) {
    int sum = 0;
    for (int i = 0; i < n; i += 1)
        sum += arr[i];
    int target = (sum - diff) / 2;
...
int main() {
    ...
    cout << "Enter given target sum: ";
    cin >> sum;
    // cout << "TargetSum " << ": " << TargetSum(arr, n, sum) << "\n";
    cout << "TargetSum " << ": " << TargetSumOptimized(arr, n, sum) << "\n";
    return 0;
}